...
bool checkGitHubUpdate() {
  ...
  if (!http.begin(client, GITHUB_RELEASE_API)) { firmwareStatus = "error"; firmwareError = "Could not start GitHub request"; return false; }
  http.addHeader("Accept", "application/vnd.github+json"); http.addHeader("User-Agent", "TubemanController/" FIRMWARE_VERSION);
  int code = http.GET();
  if (code != HTTP_CODE_OK) { firmwareStatus = "error"; firmwareError = "GitHub returned HTTP " + String(code); http.end(); addLog("Firmware check failed: " + firmwareError); return false; }
  String body = http.getString(); http.end();
  latestVersion = jsonField(body, "tag_name");
  int pos = 0;
  while (true) {
    int namePos = body.indexOf("\"name\"", pos); if (namePos < 0) break;
    String name = jsonField(body, "name", namePos);
    if (name == GITHUB_ASSET_NAME) { latestDownloadUrl = jsonField(body, "browser_download_url", namePos); break; }
    pos = namePos + 6;
  }
  if (!latestVersion.length()) { firmwareStatus = "error"; firmwareError = "Latest release has no version tag"; addLog("Firmware check failed: " + firmwareError); return false; }
  if (!latestDownloadUrl.length()) { firmwareStatus = "error"; firmwareError = "Latest release is missing tubeman-controller.bin"; addLog("Firmware check: " + firmwareError); return false; }
